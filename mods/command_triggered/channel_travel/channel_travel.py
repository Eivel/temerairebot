...
class ChannelTravel():
    ...
    def __init__(self, telegram_config: TelegramConfig, ct_config: ChannelTravelConfig):
        ...
        self.whitelisted_usernames = ct_config.whitelist
        self.message_text = ct_config.message

        self.telegram_client = TelegramClient(telegram_config.telegram_api_session_name, telegram_config.telegram_api_id, telegram_config.telegram_api_hash).start()

    def command_channels(self, update, context):
        if not update.message.from_user.username.lower() in self.whitelisted_usernames:
    ...
    def callback_channels(self, update, context):
        if not update.callback_query.from_user.username.lower() in self.whitelisted_usernames:
            return
